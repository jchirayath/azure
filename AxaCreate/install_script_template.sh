...
FQDN=$(hostname -d)
sudo postconf -e "myhostname = $myVM_HOST.$myVM_REGION"
sudo postconf -e "mydestination = $myVM_HOST.$myVM_REGION, localhost.localdomain, localhost"
...
## Test MySQL
echo "## Testing MySQL"
sudo mysql -u root -p"$myMYSQL_ADMIN_PASSWORD" -e "SHOW DATABASES;"
...
sudo certbot --nginx -d "$myVM_HOST.$myVM_REGION.cloudapp.azure.com" --non-interactive --agree-tos --email "$myEMAIL_USER"
sudo certbot --apache -d "$myVM_HOST.$myVM_REGION.cloudapp.azure.com" --non-interactive --agree-tos --email "$myEMAIL_USER"
sudo certbot certonly --standalone -d "$myVM_HOST.$myVM_REGION.cloudapp.azure.com" --non-interactive --agree-tos --email "$myEMAIL_USER"
